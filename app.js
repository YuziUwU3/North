...
function pfMsgPreview(m){const p=pfMsgPayload(m);if(!p)return (m&&m.text)||'';if(p.type==='sticker')return '[表情]'+(p.meaning?(' '+p.meaning):'');if(p.type==='image')return '[图片]'+(p.desc?(' '+p.desc):'');if(p.type==='transfer')return '[转账] ¥'+(+p.amount||0).toFixed(2);if(p.type==='redpacket')return '[红包] ¥'+(+p.amount||0).toFixed(2);return '[消息]';}
...
function pfStoreMessage(m){const p=phoneFriendState();if(!m)return;const me=p.id;const from=(''+(m.from_id||m.from||'')).toUpperCase(),to=(''+(m.to_id||m.to||'')).toUpperCase();
  ...
  p.messages[other].push({id,from,to,text:pfSafeBody(m.body||m.text||''),time:ts,recalled:!!m.recalled||!!(oldLocal&&oldLocal.recalled),received:!!m.received||!!(oldLocal&&oldLocal.received),receivedBy:m.received_by||m.receiver_id||(oldLocal&&oldLocal.receivedBy)||''});
...
function openGames(){openModal(`<h3>游戏大厅</h3><div class="hint">选个游戏，再选个角色发出【邀请卡片】，等ta在微信里同意后，点卡片上的「进入游戏」就能一起玩～ 随时说"游戏结束"或点右上角「结束」就能退出。</div>
  ${GAMES.map(g=>`<div class="section"><div class="it" onclick="pickGameChar('${g.k}')">${g.e} ${g.n}<span class="v">›</span></div></div>`).join('')}
  <button class="btn g" style="margin-top:8px" onclick="closeModal()">关闭</button>`);}
