...
assert.match(manual, /replyVisibleAssistantCount\(id,aid\)===before&&!rolePhoneInspectionBlocksOrdinary\(id\)&&manualReplyRetryAllowed/);
...
assert.match(manual, /try\{if\(rolePhoneInspectionBlocksOrdinary\(id\)\)return true;await aiReply/, "an inspection already active before manual reply must be treated as normal takeover");
...
assert.match(manual, /!rolePhoneInspectionBlocksOrdinary\(id\)&&rolePhoneInspectionEpoch\(\)===inspectionEpoch&&manualReplyRetryAllowed/, "the generic empty-model hint is allowed only when no inspection happened");
