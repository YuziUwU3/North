...
test('all three selectable ringtones are bundled, distinct short chimes, and offline-ready', () => {
  ...
  for (const [key, file] of ringtoneChoices) {
    ...
    assert.ok(activeWindows >= 60, `${key} must behave like a repeating chime rather than isolated one-second beeps`);
...
test('default ringtone repeats the same soft decay used by the WeChat message ding', () => {
  ...
  const start = Math.round(0.18 * rate);
  const count = Math.round(0.28 * rate);
  let dot = 0, actualEnergy = 0, expectedEnergy = 0;
  for (let i = 0; i < count; i++) {
    ...
    const expected = attack * release * Math.sin(2 * Math.PI * 880 * elapsed);
    const actual = wav.readInt16LE(44 + (start + i) * 2) / 32767;
    dot += actual * expected;
