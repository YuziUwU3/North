...
function renderOrders() {
  ...
  root.innerHTML = orders.map((order) => {
    const provider = order.provider === 'wechat' ? '微信' : '支付宝';
    const service = Number(order.points || 0) === 0;
    return `<article class="order ${order.status === 'pending' && order.review_status === 'submitted' ? 'pending' : ''}" id="order-${esc(order.id)}">
      <div class="order-head">
        <div><div class="order-title">${provider} ¥${Number(order.amount_cny || 0).toFixed(2)} · ${service ? '音色克隆' : Number(order.points || 0).toLocaleString() + ' 点'}</div><div class="sub">订单 ${esc(shortId(order.id))}</div></div>
...
window.openApprove = (id) => {
  ...
  openSheet(`<h2>确认实际到账</h2>
    <p>请先在微信或支付宝账单中核对金额、时间和付款人。确认后，${Number(order.points || 0).toLocaleString()} 点会立即进入 AI 账户 <b>${esc(order.user_id)}</b>。</p>
...
async function enableNotifications() {
  ...
  try {
    ...
    const registration = await navigator.serviceWorker.register('./sw.js?v=535', {scope:'./'});
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=535', {scope:'./'}).catch(() => {});
