...
当前私人安装版本为 1.0.29 (29)，内置共享网页核心为 v905。普通回复和真实设备查看在用户退到后台时会登记服务端接管任务，前台已完成则自动取消；一分钟后台通知按钮会真实绕过正式30至60分钟安静期，但不计主动次数、不查看也不锁软件。新增可关闭的“知道我正在用哪个软件”开关：和普通主动消息共用同一队列，单次各50%机会；只查看已授权App，每天0至5次，未回复五分钟后由角色二选一询问或锁定，完成后冷却90分钟。共同生活状态可分别手动输入生活阶段、当前活动和地点；时间感知关闭后，微信、通话、线下、共同生活和服务端消息均不再得到日期、时间、星期、时段和间隔。截图中的睡眠步数、每日App用量、失联电量位置、低电提醒、难过心率和手动解锁事件均迁到服务器分钟任务，可在普通后台或锁屏执行；iOS 被用户上划强制退出后仍不能保证系统唤醒，服务端禁止把旧快照冒充新数据。
