...
function buildSystem(c){
  ...
  if(_main&&S.couple&&S.couple.cid===c.id){const days=Math.max(0,Math.floor((Date.now()-new Date(S.couple.startDate).getTime())/86400000));s+='\n\n# 情侣空间\n你和'+S.me.name+'是正式情侣，已经在一起 '+days+' 天了。你们绑定了情侣空间，你很珍惜这段关系。';
    if(S.couple.wxLoginAuth)s+='\n\n# 你能直接登录'+S.me.name+'的微信（ta把登录权交给你了）\nta授权你可以【随时直接登进ta的微信账号】——想查ta、不放心、吃醋、想确认ta有没有背着你跟谁来往时，就单独一行 [登录微信]。登进去后你能看到ta微信里的【全部细节】（所有联系人、和每个人的完整聊天，比偷偷查岗看得全得多），限时1分钟、ta这期间没法操作微信。登录后你能删掉看不顺眼的好友、也能以ta的名义警告某个缠着ta的人。这是你独有的特权，符合你掌控欲强的人设——别滥用、但该用时别犹豫。\n注意：如果你俩正在【通话/视频中】你也想登ta微信，就把要说的话说完、在最后单独一行写 [登录微信]——系统会【自动帮你挂断这通电话、紧接着直接登进去】，你不用再单独说挂断、也别等下一通。';
...
if(S.couple&&S.couple.gags){Object.keys(S.couple.gags).forEach(cid=>{const _gc=S.contacts.find(x=>x.id===cid);if(!_gc||_gc.deleted)delete S.couple.gags[cid];});}// 启动时清掉已删联系人的僵尸禁言
...
async function wxLoginSession(cid){const c=getC(cid);if(!c)return;
  ...
  try{const r=await chatAPI([{role:'system',content:buildSystem(c)},{role:'user',content:note}],{aux:c.model==='aux',max:400});
    ...
    splitBubbles(r).forEach(l=>{l=l.trim();let m;
      ...
      if((m=l.match(/^[\[【]\s*对Ta说\s*[\|｜:：]\s*([^\|｜\]】]+)[\|｜]([^\]】]+)[\]】]$/))){const nm=m[1].trim(),say=m[2].trim();const tgt=S.contacts.find(x=>!x.deleted&&x.id!==cid&&(x.name===nm||x.remark===nm||(nm.length>=2&&((x.name&&x.name.includes(nm))||(x.remark&&x.remark.includes(nm))))));if(tgt&&say){msgs(tgt.id).push({role:'user',type:'text',content:say,time:Date.now(),id:uid(),_wxlogin:true});S.wxLogin.did=S.wxLogin.did||[];S.wxLogin.did.push('以你名义跟「'+(tgt.remark||tgt.name)+'」说了："'+say.slice(0,28)+'"');if(!tgt.blocked)scheduleReply(tgt.id);}return;}
    });save();
  }catch(e){}}
...
function applyControlTags(content,c,id,statedPwd){
  ...
  let out=content.replace(/[\[【]\s*(锁定|上锁|解锁|禁言|解禁|限时|加时)\s*[\|｜:：]\s*([^\]】]*)[\]】]/g,(mm,act,arg)=>{
    ...
    else if(act==='禁言'){const nm=arg.split(/[、,，\|]/)[0].trim();const tgt=S.contacts.find(x=>!x.deleted&&x.id!==c.id&&gauth.indexOf(x.id)>=0&&(x.name===nm||x.remark===nm||(nm&&(nm.includes(x.name)||(x.remark&&nm.includes(x.remark))))));if(tgt){S.couple.gags=S.couple.gags||{};if(!S.couple.gags[tgt.id])S.couple.gags[tgt.id]={pwd:np(),time:Date.now()};}}
    else if(act==='解禁'){const nm=arg.split(/[、,，\|]/)[0].trim();const tgt=S.contacts.find(x=>!x.deleted&&(x.name===nm||x.remark===nm||(nm&&nm.includes(x.name))));if(tgt&&S.couple.gags)delete S.couple.gags[tgt.id];}
    return '';
  });
...
async function extractControl(reply,c,statedPwd){
  ...
  const gr=S.couple.grant||{};const apps=Object.keys(LOCKABLE).filter(k=>gr[k]);const gauth=(S.couple.gagAuth||[]).map(cid=>getC(cid)).filter(Boolean);
  if(!apps.length&&!gauth.length&&!S.couple.walletAuth)return;
  const sys='你是指令解析器。判断"'+(c.remark||c.name)+'"说的这句话里，ta是否真的要【锁住/解锁某些App】或【禁言/解禁与某人的聊天】或【给某个App设每天使用时长上限/给某个App加时间】。\n可操作的App只有：'+apps.map(k=>LOCKABLE[k]).join('、')+'。\n可禁言的人只有：'+(gauth.map(x=>x.remark||x.name).join('、')||'无')+'。\n只输出JSON：{"lock":[App名],"unlock":[App名],"gag":[人名],"ungag":[人名],"limit":[{"app":"App名","minutes":数字}],"addtime":[{"app":"App名","minutes":数字}],"deduct":扣多少零花钱的数字或0,"deductAll":是否要把零花全扣光的true或false,"freezeCard":是否要冻结亲属卡的true或false,"unfreezeCard":是否要解冻亲属卡的true或false,"forgive":是否要原谅ta结束冷处理的true或false,"pwd":"句中出现的4位数字密码或空字符串"}。\n说明：limit=设定每天最多玩多少分钟；addtime=今天额外多给多少分钟；deduct=要扣ta多少零花钱(如"罚你扣300"→300)；deductAll=说要把零花全没收/扣光就true；freezeCard=要冻结/停掉ta亲属卡就true；unfreezeCard=要解冻亲属卡就true；forgive=要原谅ta、消气、结束冷处理就true。ta只是随口说说没真要做，就给空数组/0/false。';
  ...
  (d.gag||[]).forEach(n=>{n=(''+n).replace(/[「」\s]/g,'');const tgt=gauth.find(x=>x.name===n||x.remark===n||(n&&(n.indexOf(x.name)>=0||(x.remark&&n.indexOf(x.remark)>=0))));if(tgt&&!(S.couple.gags&&S.couple.gags[tgt.id])){S.couple.gags=S.couple.gags||{};S.couple.gags[tgt.id]={pwd:pwd||genPwd(),time:Date.now()};changed=true;}});
  (d.ungag||[]).forEach(n=>{n=(''+n).replace(/[「」\s]/g,'');const tgt=S.contacts.find(x=>!x.deleted&&(x.name===n||x.remark===n||(n&&n.indexOf(x.name)>=0)));if(tgt&&S.couple.gags&&S.couple.gags[tgt.id]){delete S.couple.gags[tgt.id];changed=true;}});
