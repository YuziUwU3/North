...
assert.match(account,/function aiRenderStable\(\)/);
assert.match(account,/_aiVoiceTestStatus='语音生成中[^;]+;if\(cur\(\)\.p==='aiaccount'\)aiRenderStable\(\)/);
assert.match(account,/finally\{_aiVoiceTestBusy=false;if\(cur\(\)\.p==='aiaccount'\)aiRenderStable\(\);\}\}/);
assert.match(account,/if\(cur\(\)\.p==='aiaccount'\)setTimeout\(\(\)=>\{if\(cur\(\)\.p==='aiaccount'\)aiRenderStable\(\);\},30\);\}/);
