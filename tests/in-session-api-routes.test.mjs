...
assert.match(functionSource('chatAPI'),/if\(chatRouteSessionPage\(\)\)opt\.aux=false/);
...
assert.match(source,/app\.innerHTML='<div class="page'\+_wxL\+'">'\+html\+'<\/div>';\s*chatRouteMount\(c\);/);

const calls=[];
const context=vm.createContext({
  S:{settings:{chat:{base:'https://primary.example/v1',key:'primary-key',model:'primary-model',temp:.7,maxTokens:700},aux:{base:'https://aux.example/v1',key:'aux-key',model:'aux-model'}}},
  cur:()=>({p:'off'}),
  aiCoreOn:()=>false,
...
context.cur=()=>({p:'chat'});
await context.chatAPI([],{aux:true});
assert.equal(calls[1].url,'https://aux.example/v1/chat/completions');
assert.equal(calls[1].body.model,'aux-model','other auxiliary tasks must keep their existing routing');
