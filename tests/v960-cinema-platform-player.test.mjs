...
test('platform parser accepts only canonical Youku and Bilibili video identifiers',()=>{
...
test('Youku search and playback use current official endpoints without stream scraping',()=>{
  ...
  assert.doesNotMatch(search+player,/api\.bilibili\.com|playurl|m3u8|dash|cookie|Authorization|解锁|解析视频/i);
...
test('official platform playback keeps role chat honest and preserves the existing sources',()=>{
  ...
  assert.match(functionSource('cinemaAnalyzeFrame'),/平台官方播放器保护画面/);
