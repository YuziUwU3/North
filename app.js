...
function charImgPrompt(c,desc){
  ...
  const objectHit=/(小猫|猫|狗|宠物|动物|物品|东西|礼物|商品|摆件|戒尺|衣服|裙子|鞋|包|书|文件|杯子|饭|菜|咖啡|风景|窗外|房间|桌面|工具|器具|电脑|键盘|屏幕|花|车|门|床|沙发|拍一下|拍一张|给我看看)/.test(scene);
  const activityOnly=/(你在干嘛|你在干吗|在干嘛|在干吗|干什么|忙什么|做什么|看看你在|发张图看看你|发张图片看看你)/.test(scene);
  const wantsSelf=/(自拍|本人|你本人|你自己|把你|拍你|你也入镜|你入镜|露脸|侧脸|背影|半身|全身|身材|肌肉|腹肌|穿着|衣服|低头|侧影|镜子|镜中|男友视角|给我看看你本人|给我看看你自己)/.test(scene)&&!objectHit&&!activityOnly;
  const objectOnly=objectHit||activityOnly;
  ...
  if(objectOnly){
    ...
    else p+='画面主体必须是物品、动物或场景本身，不要出现他本人、脸、头发、身体、手、镜中自拍、手机遮脸的人影，也不要为了“不露脸”硬塞一个被遮挡的人。';
  }else{
