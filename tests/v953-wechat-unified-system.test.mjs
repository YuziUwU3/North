...
assert.match(source,/APP_VER='v955 · 限额与生活事件连续性修复'/);
assert.match(html,/__NORTH_SHELL_BUILD__='955'/);
assert.match(sw,/BUILD='955'/);
assert.equal((project.match(/CURRENT_PROJECT_VERSION = 77;/g)||[]).length,12);
assert.equal((project.match(/MARKETING_VERSION = 1\.0\.77;/g)||[]).length,12);
...
console.log('v955 WeChat unified system tests passed');
