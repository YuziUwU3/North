...
test('v906 and private 1.0.30 versions are aligned', () => {
  assert.match(app, /APP_VER='v906 · 后台实测与宠物健康'/);
  assert.match(html, /app\.js\?v=906/);
  assert.match(project, /CURRENT_PROJECT_VERSION = 30;/);
  assert.match(project, /MARKETING_VERSION = 1\.0\.30;/);
  assert.match(nativeWeb, /1\.0\.30 \(30\)/);
});
