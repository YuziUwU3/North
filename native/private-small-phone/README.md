...
当前私人安装版本为 1.0.38 (38)，内置共享网页核心为 v914，原生桥契约为 16。共享状态仍由原生每半秒核验并由网页主动补偿轮询；实时共享理解开关会立即同步设置页外观，共享按钮在等待系统“开始直播”确认时显示等待态。完整通话页恢复实心字幕布局且空字幕不占位；仅系统 PiP 悬浮通话小窗使用更低透明度背景，以尽量显露 iPhone 主屏幕或其他 App。音乐新增无需用户登录的 Audius 公开曲库搜索，收藏后继续走原有一起听上下文。Mac 六 Target 编译、签名、PiP 实际透明合成效果及真实 iPhone 锁屏/切其他 App 后的持续共享理解仍需最终验收。
