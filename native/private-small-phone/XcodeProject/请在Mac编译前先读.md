...
- 私人版本为 1.0.12 (12)；共享网页核心仍为 v887。当前审核中的公开 North App 工程仍未改动。
- 私人 App 已增加手机号验证码登录、Keychain 登录态以及按账号隔离的云备份／恢复；首次绑定不会静默覆盖本机数据。编译前仍需先在 Supabase 启用 Phone Provider、配置短信服务并执行 `supabase/migrations/202608110001_private_phone_accounts.sql`，否则验证码入口会明确报未启用。
...
- v881 已建立永久锁定账本、当日快照字段、前台授权复检和真实状态措辞；v887 继续保留这些逻辑。私人 1.0.11 禁止 SwiftUI 根据键盘安全区缩小 WKWebView 外框，只保留 WebKit 自己的一次聚焦处理，消除开合键盘时先下拉再上弹的双重动画；1.0.12 在此稳定基线上增加手机号账号和云恢复，角色播音结束后的延迟强制重建原生录音会话继续保留。
