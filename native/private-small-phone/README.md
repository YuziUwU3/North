...
当前私人安装版本为 1.0.10 (10)，内置共享网页核心 v886。真机证明原生专属固定输入框和 contentOffset 锁会与 WKWebView 键盘动画形成两次布局；电话页现已撤掉这两层覆盖，完整复用网页版已验证顺滑的“整个页面随键盘上移/回落”布局，输入框在上、挂断按钮在下。角色播放完成后的原生录音强制重建继续保留。
