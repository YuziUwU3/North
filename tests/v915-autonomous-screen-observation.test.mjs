...
test('realtime screen observation has explicit quiet, question and end decisions',()=>{
  ...
  assert.match(app,/function callScreenAutonomyApply\(decision\)/);
...
test('a user answer resumes or ends observation through the normal call turn',()=>{
  assert.match(app,/function callScreenAutonomyUserAnswered\(text\)/);
  assert.match(app,/const autonomyAnswer=callScreenAutonomyUserAnswered\(t\)/);
