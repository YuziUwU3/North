...
test('v969 uses one web and private iOS build identity', () => {
  assert.match(app, /APP_VER='v969 · Xcode编译与人声增益修复'/);
  assert.match(app, /sw\.js\?v=969&r=v969-native-build-gain-1/);
  assert.match(shell, /north-shell-v969/);
  assert.match(bundleInfo, /<string>969<\/string>/);
  assert.match(localWebView, /1\.0\.91 \(91\)/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 91;/g) || []).length, 12);
  assert.equal((project.match(/MARKETING_VERSION = 1\.0\.91;/g) || []).length, 12);
});
