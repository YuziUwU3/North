...
当前私人安装版本为 1.0.22 (22)，内置共享网页核心为 v897。用户提出真实读取时，微信聊天和通话会在调用普通模型前进入独占读取通道；同一用户消息不能再并发启动第二次回复。顶部横幅负责逐项显示“正在读取什么”，原生同一轮完成后才允许角色用自己的语气自然说一次。v896 的固定九项报告已撤销，成功事实只作为隐藏上下文，未返回项目和技术失败原因不会再冒充角色消息。共同生活沿用同一完成后自然回复规则。Windows 475 项自动测试通过；Mac 编译、签名和真机全链路验收仍未完成。
