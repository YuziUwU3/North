...
- 私人版本为 1.0.13 (13)；共享网页核心为 v888。当前审核中的公开 North App 工程仍未改动。
...
- v881 已建立永久锁定账本、当日快照字段、前台授权复检和真实状态措辞；v888 继续保留这些逻辑。私人 1.0.11 禁止 SwiftUI 根据键盘安全区缩小 WKWebView 外框，只保留 WebKit 自己的一次聚焦处理，消除开合键盘时先下拉再上弹的双重动画；1.0.13 在账号和云恢复基线上改用手机号密码登录，角色播音结束后的延迟强制重建原生录音会话继续保留。
