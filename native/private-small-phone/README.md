...
当前私人安装版本为 1.0.14 (14)，内置共享网页核心为 v889。1.0.11 已通过固定 WKWebView 外框解决键盘跳动；1.0.13 在账号与云恢复基础上改用“手机号 + 密码”；1.0.14 把所有核心状态强制写入 Application Support 原生存档，并增加原子双副本、旧快照拦截、损坏回退和真实手机剩余容量显示。界面仍只显示手机号；密码不保存，访问令牌与刷新令牌只存 iPhone Keychain。首次登录绝不自动覆盖已经导入的本机数据。电话页和连续原生语音修复继续保留。
