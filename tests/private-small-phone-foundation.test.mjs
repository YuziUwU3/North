...
test('private app loads bundled phone resources instead of a remote shell', () => {
  ...
  assert.match(webView, /__SMALL_PHONE_PRIVATE_BUILD__ = '1\.0\.29 \(29\)'/);
...
test('real Mac project keeps all Screen Time targets and becomes 小手机', () => {
  ...
  assert.match(project, /CURRENT_PROJECT_VERSION = 29;/);
  assert.match(project, /MARKETING_VERSION = 1\.0\.29;/);
