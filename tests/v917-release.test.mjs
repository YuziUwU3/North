...
test('v919 web and private 1.0.43 versions are aligned', () => {
  assert.match(app, /APP_VER='v919 · 通话声音与输入框回归修复'/);
  assert.match(html, /__NORTH_SHELL_BUILD__='919'/);
  assert.match(sw, /const BUILD='919'/);
  assert.doesNotMatch(project, /CURRENT_PROJECT_VERSION = 40|MARKETING_VERSION = 1\.0\.40/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 43;/g) || []).length, 12);
  assert.equal((project.match(/MARKETING_VERSION = 1\.0\.43;/g) || []).length, 12);
  assert.match(webView, /__SMALL_PHONE_PRIVATE_BUILD__ = '1\.0\.43 \(43\)'/);
});
