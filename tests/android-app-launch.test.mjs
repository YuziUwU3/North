...
assert.match(source, /const APP_VER='v963 · 陪看字幕、共享画面与后台链路修复'/);
...
assert.match(source, /if\(!p\.pan&&Math\.max\(ax,ay\)>12\)\{clearTimeout\(_aTimer\);p\.pan=ax>ay\?'x':'y'/);
assert.match(source, /function appPanMove\(x,y,e\)[\s\S]*?if\(p\.pan\)\{if\(e&&e\.cancelable\)e\.preventDefault\(\);if\(p\.pan==='x'/);
assert.match(source, /function appMove\(e\)[\s\S]*?appPanMove\(e\.clientX,e\.clientY,e\)/);
assert.match(source, /function appTouchMove\(e\)[\s\S]*?if\(_aPend\)appPanMove\(t\.clientX,t\.clientY,e\)/);
assert.match(source, /if\(p&&p\.pan\)\{if\(p\.pan==='x'&&p\.sw\)homeSnapPage\(p\.sw\);return;\}/);
