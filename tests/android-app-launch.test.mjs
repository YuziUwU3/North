...
assert.match(source, /const APP_VER='v927 · 苹果首屏文字、安全区与美化导入修复'/);
...
assert.match(source, /if\(Math\.abs\(dx\)>12&&Math\.abs\(dx\)>Math\.abs\(dy\)\*1\.15\)\{clearTimeout\(_aTimer\);_aPend=null;return;\}/);
assert.match(source, /if\(Math\.abs\(dx\)>APP_TAP_MOVE\|\|Math\.abs\(dy\)>APP_TAP_MOVE\)/);
