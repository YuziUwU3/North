...
test('Moments content is formally integrated without replacing its data or actions',()=>{
  ...
  assert.match(html,/\.wx-premium\.wx-moments:not\(\.wxlight\)>\.scroll>\.mpost\{[^}]*background:#19191b/);
  ...
  assert.match(app,/toggleMomentLike\('\$\{p\.id\}'\)/);
  assert.match(app,/momentCommentFocus\('\$\{p\.id\}'\)/);
});
