...
test('v921 web and private 1.0.45 versions are aligned', () => {
  assert.match(app, /APP_VER='v921 · 主屏幕通话字幕恢复'/);
  assert.match(html, /__NORTH_SHELL_BUILD__='921'/);
  assert.match(sw, /const BUILD='921'/);
  assert.doesNotMatch(project, /CURRENT_PROJECT_VERSION = 40|MARKETING_VERSION = 1\.0\.40/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 45;/g) || []).length, 12);
  assert.equal((project.match(/MARKETING_VERSION = 1\.0\.45;/g) || []).length, 12);
  assert.match(webView, /__SMALL_PHONE_PRIVATE_BUILD__ = '1\.0\.45 \(45\)'/);
});
