...
当前私人安装版本为 1.0.36 (36)，内置共享网页核心为 v912，原生桥契约为 15。共享语音冻结系统画面后，原生端会为该帧令牌申请有限后台执行时间，网页完成识图与本轮角色回复后通过新桥命令成对结束；无共享帧令牌的摄像头识图不等待回复，保持原来的快速路径。主页面字幕强制重播原有 `csin .3s ease`，悬浮通话字幕使用同一 0.3 秒、8px 位移和 CSS `ease` 控制点。闹钟备注会提前生成角色口吻提醒并和 AlarmKit 同步排期，到点由本地通知可靠显示，之后从已送达或已点击事件补入聊天且按闹钟与日期去重。Mac 六 Target 编译、签名及真实 iPhone 后台识图、悬浮字幕和闹钟到点通知/聊天补记仍需最终验收。
