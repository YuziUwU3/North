...
当前私人安装版本为 1.0.37 (37)，内置共享网页核心为 v913，原生桥契约为 16。共享状态由原生每半秒核验并由网页主动补偿轮询，开始/结束事件使用独立队列且会作废旧回复。新增默认关闭的实时共享理解：ReplayKit 帧序列变化时约每 4 秒最多通知一次，网页先比较 12×12 亮度签名，只有明显变化才进入识图；每帧识别使用有限后台任务，角色回复完成后成对释放。自动识别次数继续共用单次视频上限，口头识图不限次。展开通话框和 PiP 字幕统一为 0.3 秒、8px 的自然渐显上移，PiP 保持 360 宽、缩至 176 高并使用轻透明背景。Mac 六 Target 编译、签名及真实 iPhone 锁屏/切其他 App 后的持续共享理解仍需最终验收。
