...
当前私人安装版本为 1.0.16 (16)，内置共享网页核心为 v891。1.0.11 已通过固定 WKWebView 外框解决键盘跳动；1.0.13 在账号与云恢复基础上改用“手机号 + 密码”；1.0.14 把所有核心状态强制写入 Application Support 原生存档，并增加原子双副本、旧快照拦截、损坏回退和真实手机剩余容量显示；1.0.15 让手机号账号接管私人小手机的唯一伴生设备控制权；1.0.16 让角色查看前直接唤醒外置 iPhone 并等待新快照，新增“一键查看全部数据”，同时用原生桥接接管定位并只在首次请求屏幕使用时间权限。界面仍只显示手机号；密码不保存，访问令牌与刷新令牌只存 iPhone Keychain。首次登录绝不自动覆盖已经导入的本机数据。电话页和连续原生语音修复继续保留。
