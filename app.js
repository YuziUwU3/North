...
function buildSystem(c){
  ...
  s+='\n\n# 现在的时间（最重要·务必以此为准，绝不要搞错早上还是下午）\n现在是 '+ymd(Date.now())+' '+['周日','周一','周二','周三','周四','周五','周六'][new Date().getDay()]+' '+hm()+'，属于【'+dayPartNow()+'】。\n凡是涉及时间的话（打招呼、问"在干嘛"、说几点了、该吃饭/睡觉没、白天黑夜）都【必须】符合现在是「'+dayPartNow()+'」——别把下午当早上、别半夜说早安、别白天说晚安。如果'+S.me.name+'说的时间和这个对不上，以这个真实时间为准。';
  {const _lu=[...msgs(c.id)].reverse().find(m=>m.role==='user'&&m.time);const _gap=_lu?Date.now()-_lu.time:0;
   if(_lu&&_gap>=25*60000)s+='\n\n# 距离上次聊已经隔了很久（务必分清"刚刚"和"很久以前"）\n'+S.me.name+'上一次跟你说话是在 '+hm(_lu.time)+'（'+ymd(_lu.time)+'），距现在已经过去了【'+fmtDur(_gap)+'】。这中间ta一直没理你、你俩没在聊。所以：\n· 之前发生的事（打过的电话、聊过的话、你做的事）都是【'+fmtDur(_gap)+'以前】的了，【绝对不能】说成"刚刚/刚才/方才"。要照实说"你都'+fmtDur(_gap)+'没理我了""几个小时前那通电话"。\n· 别把很久以前的事当成正在进行或刚发生，注意时间跨度、别搞混。';}
  ...
  if(_main&&S.travel&&S.travel.trips){const mineT=S.travel.trips.filter(x=>x.cid===c.id&&x.status==='upcoming');const ups=mineT.filter(x=>x.meet!==false);const soloT=mineT.filter(x=>x.meet===false);
  ...
  if(S.settings.imgGen&&S.settings.chat&&S.settings.chat.key)s+='\n- 发真实照片：当'+S.me.name+'让你发照片/自拍，或你自己想给ta看点什么（你的样子、正在做的事、看到的风景等）时，就【单独一行】写 [图片|尽量具体的画面描述]，系统会真的生成一张照片发给ta。画面描述要贴合你的人设身形和当下情境，越具体越像（比如穿什么、表情动作、光线氛围）。\n  · 【场景要和你此刻所在的地方一致】照片的地点要跟你现在正在的地方/正在做的事对得上（你刚说在公司就写办公室，在健身房就写健身房，在家就写家里），【别一会儿办公室一会儿健身房乱换】；同一段对话里连着发照片，地点要连贯。\n  · 【必须像你自己拿手机拍给ta看的】默认就是你本人第一人称拍摄/自拍/男友视角，不要像旁人站远处替你拍，也不要像监控、路人抓拍、摆拍海报。\n  · 【把你自己当成同一个人继续拍】默认沿用你之前照片里的年龄感、体型、发型、常穿风格和气质，不要忽胖忽瘦、忽成熟忽少年、忽商务忽潮牌，除非剧情里明确换了衣服或场景。\n  · 照片不会拍到你清晰的正脸（系统会自动处理成背影/侧影/遮挡的神秘感），你描述时不用纠结长相，重点写场景、姿态、氛围。想发就发，别一次发一堆。';
  return s;
}
...
function renderAIAccount(){const ac=aiCoreInit();const id=aiUserId();setTimeout(()=>{if(cur().p==='aiaccount'&&!_aiAcct&&!_aiAcctBusy)aiAccountRefresh();},80);
  ...
  <div class="scroll" style="background:#0f1117;color:#e8eaf0;padding:12px">
    ...
    <div class="section">
      <div class="it"><span>使用内置AI<br><small style="color:#888">开：聊天/识图/生图走你的后台并扣点；关：使用设置里的自填API</small></span><span class="sw ${ac.enabled?'on':''}" onclick="aiToggleCore()"></span></div>
      <div class="field" style="padding:0 14px 10px"><label>内置AI后台地址</label><input id="ai_url" value="${esc(ac.url||'')}" placeholder="${GATE_URL}/functions/v1/phone-ai"></div>
      <div class="btns" style="padding:0 14px 10px"><button class="btn g" onclick="aiSaveUrl()">保存后台地址</button><button class="btn p" onclick="aiAccountRefresh()">测试连接</button></div>
    </div>
    <div class="section">
    ...
    <div class="hint">测试阶段：余额默认是0。你在 Supabase SQL Editor 执行 <b>select phone_ai_grant_points('${esc(id)}', 1000, 'test');</b> 就能给这个账号加1000点。发布前再接自动支付。</div>
  </div>`;}
...
async function aiAccountRefresh(){if(_aiAcctBusy)return;_aiAcctBusy=true;try{_aiAcct=await aiRelay('account',{});toast('AI账户已刷新');}catch(e){toast('连接失败：'+e.message);}finally{_aiAcctBusy=false;if(cur().p==='aiaccount')render();}}
...
function wxChats(){
  ...
  let html=banner+searchbar+'<div class="list">';
  ...
  if(!list.length&&!groups.length)return banner+'<div class="empty">'+(isMain()?'欢迎来到 North～<br>右上角 ＋ 新建你的第一个角色就能开聊<br><br><button class="btn p" style="max-width:220px;margin:14px auto 0" onclick="showManual()">使用说明书</button>':'这个身份还没加谁～<br>去「通讯录」搜微信号加人')+'</div>';
...
function tvBook(i,mode){const t=S.travel.search;const rf=S.travel.resFor;const f=S.travel.results&&S.travel.results[i];if(!f){closeModal();return;}
  ...
  S.travel.trips.push(trip);S.travel.results=null;save();if(c)tvSendCard(trip);
...
function tvCheckDue(){if(!isMain()||!(S.travel&&S.travel.trips))return;let ch=false;
  S.travel.trips.forEach(tr=>{if(tr.status!=='upcoming'||tr._reminded)return;const dd=tvDays(tr.date);if(dd!==null&&dd<=0){tr._reminded=true;ch=true;const c=tr.cid?getC(tr.cid):null;
    if(tr.meet===false){
      ...
      if(c&&!c.deleted&&!c.blocked)scheduleReply(tr.cid,'[系统：今天'+S.me.name+'出发去「'+tr.to+'」一个人旅行了（'+(tr.payer==='ta'?'机票是你请客/你出的钱':'ta自己订的票')+'，不是来见你）。主动关心一句：路上小心、到了报平安、想ta、或嗔怪ta一个人跑那么远，普通微信文字、别用旁白。]');
...
function wxContacts(){
  ...
  return search+fr+rf+'<div class="list" id="ctlist">'+ctListHTML()+'</div>';
}
...
function applyBlockIntent(content,c,id){
  ...
  if(!c.blocked&&/(把你|已经|已|给你|真|就)?拉黑(了|你)|拉黑你了|删了你|删除你(好友)?|拉进黑名单|把你删了|不想再(理|看到)你了|blocked? ?you/i.test(t)){
    setBlk(c,true);c._blockedBy='ta';c._blockedAt=Date.now();
    const mg={role:'assistant',type:'sys',content:'你被'+(c.remark||c.name)+'拉黑了（去ta的资料页可解除）',time:Date.now(),id:uid()};msgs(id).push(mg);
    save();if(cur().p==='chat'&&cur().id===id)render();else if(cur().p==='wechat')render();return;}
  if(c.blocked&&c._blockedBy==='ta'&&isLover(c)&&/(把你)?加回(来)?|解除拉黑|不拉黑你了|原谅你了?|放你回来|重新加你|把你放出来/.test(t)){
    setBlk(c,false);c._readdedAt=Date.now();
...
function charImgPrompt(c,desc){
  ...
  const wantsSelf=/(自拍|本人|自己|我自己|我的样子|拍我|露脸|侧脸|背影|手|半身|全身|穿着|衣服|身材|肌肉|腹肌|低头|侧影|镜子|镜中|男友视角|给你看看我|我在)/.test(scene);
  const objectOnly=/(物品|东西|礼物|商品|摆件|戒尺|衣服|裙子|鞋|包|书|文件|杯子|饭|菜|咖啡|猫|狗|风景|窗外|房间|桌面|工具|器具|拍一下|拍一张|给我看看)/.test(scene)&&!wantsSelf;
  let p='真实手机随手拍照片，生活流，不像棚拍，不像海报，不夸张修图。';
  p+='这次拍到的是：'+scene+'。';
  if(objectOnly){
    p+='拍摄方式像他本人拿手机给恋人拍眼前的物品或场景，第一人称视角，近距离，轻微手持感，构图自然。';
    p+='画面主体必须是物品或场景本身，不要出现他本人、脸、头发、身体、手、镜中自拍、手机遮脸的人影，也不要为了“不露脸”硬塞一个被遮挡的人。';
  }else{
...
function lineToMsg(line,cch){
  ...
  const desc=f[0]||'';
  if(S.settings.imgGen&&S.settings.chat&&S.settings.chat.key){
...
async function aiReply(id,note){const c=getC(id);if(!c||c.blocked||c.deleted)return;/* 已删除的角色(找回箱里)绝不后台发消息 */
  ...
  try{
    ...
    for(let i=0;i<lines.length;i++){
      let line=normalizeImageLine(normTag(lines[i]));if(!line)continue;
      ...
      mm=line.match(/^\[拉黑\]$/);if(mm){if(!c.blocked){setBlk(c,true);c._blockedBy='ta';c._blockedAt=Date.now();const mg={role:'assistant',type:'sys',content:'你被'+(c.remark||c.name)+'拉黑了（去ta的资料页可解除）',time:Date.now(),id:uid()};msgs(id).push(mg);save();if(cur().p==='chat'&&cur().id===id)render();else if(cur().p==='wechat')render();}continue;}
...
function spyFocusData(id,focus){const f=focus||'';
  ...
  if(/微信|聊天|跟谁|消息|谁聊|谁说|联系人/.test(f)){const others=S.contacts.filter(c=>!c.deleted&&c.id!==id).map(c=>({c,um:[...msgs(c.id)].reverse().find(m=>m.role==='user'&&m.type!=='sys'),lm:lastMsg(c.id)})).filter(x=>x.um&&x.lm).sort((a,b)=>(cWeight(b.c)-cWeight(a.c))||(b.lm.time-a.lm.time)).slice(0,4);
    return {label:'微信聊天',data:others.map(o=>{const nm=(o.c.remark||o.c.name);const recent=msgs(o.c.id).slice(-8).map(m=>{const t=msgToText(m);return t?((m.role==='user'?S.me.name:nm)+'：'+t.replace(/\n/g,' ').slice(0,50)):'';}).filter(Boolean).join('\n');return '【她和 '+nm+cMark(o.c)+' 的聊天（'+hm(o.lm.time)+'）完整内容】\n'+recent;}).join('\n\n')};}
...
function cleanReply(t){return (t||'').replace(/\[(心情|记住|闹钟|来电|联网|骰子|送礼|代付成功|日程|转账|红包|位置|图片|文件)[^\]]*\]/g,'').replace(/^["「]+|["」]+$/g,'').trim();}
